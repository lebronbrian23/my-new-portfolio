<div class="max-w-7xl mx-auto px-6">
            ...
            @if ( !$work->is_api )
                <div class="project-card bg-white rounded-2xl overflow-hidden shadow-lg border border-primary/10" key="{{ $work->id }}">
                    <div class="aspect-video bg-sage/20 overflow-hidden">
                        <img src="{{ $work->image ? asset('storage/'.$work->image->url) : '' }}" alt="{{ $work->title }}"
    ...
    <div class="bg-sage/20 rounded-2xl p-8 border border-primary/10">
        ...
        <div class="grid md:grid-cols-3 gap-4">
            @foreach ( $work_content as $api_work )
                @if ( $api_work->is_api )
                    <a key="{{ $api_work->id }}" href="{{ $api_work->url }}" target="_blank" class="p-4 bg-white rounded-xl hover:shadow-lg transition-shadow border border-primary/10">
                        <div class="text-accent font-medium">{{ $api_work->title }}</div>
                        <div class="text-sm text-primary/60 mt-1">{{ $api_work->description }}</div>
                    </a>
                @endif
            @endforeach
        </div>
    ...
    <div class="text-center mt-12">
        <a href="{{ $github_link ? $github_link->link : '#' }}" target="_blank" class="inline-flex items-center gap-2 px-8 py-4 bg-primary text-cream font-medium rounded-full hover:bg-accent transition-all hover:shadow-lg">
